...
(defun find-end (jmp-lst)
  (loop with maze = (copy-list jmp-lst)
     for pos = 0 then (+ pos val)
     for val = (nth pos maze)
     while val
     do (setf (nth pos maze) (1+ val))
     summing 1 into jumps
     finally (return jumps)))

(defun main ()
  (loop for line = (read-line *standard-input* nil nil)
     while line ; stop on EOF
     collect (parse-integer line) into jump-maze
     finally (format t "~D~%" (find-end jump-maze))))
