...
(defun split-str-to-ints (str)
  (loop for ss in (cl-ppcre:split "\\s+" str)
     collect (parse-integer ss)))

(defun expand-list (list new-length &key (initial-element nil))
  "Returns a copy of LIST extended with as many INITIAL-ELEMENT as
   necessary to make it NEW-LENGTH in length"
  (if (<= new-length (length list))
      list ; nothing to do
      (append list (make-list (- new-length (length list))
			      :initial-element initial-element))))

(defun reallocate (memory &optional (memory-size (length memory)))
  "Performs a single reallocation cycle as defined above, returning
  the new memory allocation."
  (loop with new-memory = (expand-list memory memory-size
				       :initial-element 0)
     with max = (apply #'max memory)
     with max-pos = (position max memory)
     initially (setf (nth max-pos new-memory) 0)
     repeat max
     for cur-pos = (mod (1+ max-pos) memory-size) then (mod (1+
							     cur-pos) memory-size)
     for inc-val = (1+ (nth cur-pos new-memory))
     do (setf (nth cur-pos new-memory) inc-val)
     finally (return new-memory)))

(defun reallocate-until-repeated (memory &optional (memory-size (length memory)))
  "Performs memory reallocations until a repaeated allocation occurs. Returns the number of cycles executed."
  (loop for alloc = memory then (reallocate alloc memory-size)
     for prev-occurance = (position alloc allocations :test #'equal)
     until prev-occurance
     collect (copy-list alloc) into allocations
     summing 1 into count
     finally (return (list count (- count prev-occurance)))))

(defun main ()
  (loop for line = (read-line *standard-input* nil nil)
     while line ; stop on EOF
     do (format t "~D~%" (reallocate-until-repeated (split-str-to-ints line)))))
